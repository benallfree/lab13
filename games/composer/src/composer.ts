...
class MusicComposer {
  ...
  private renderGrid() {
    ...
    gridElement.style.setProperty('--cols', this.gridSize.cols.toString())
    ...
    expandBtn.className = 'col-label expand-btn'
    expandBtn.textContent = '+'
    expandBtn.title = 'Add 64 more columns'
    expandBtn.addEventListener('click', () => this.expandColumns())
    gridElement.appendChild(expandBtn)
    ...
    for (let row = 0; row < this.gridSize.rows; row++) {
      ...
      // Add empty cell for expand button alignment
      const emptyCell = document.createElement('div')
      emptyCell.className = 'cell expand-cell'
  ...
  private expandColumns() {
    const newCols = this.gridSize.cols + 64

    // Expand each row with new empty cells
    for (let row = 0; row < this.gridSize.rows; row++) {
      for (let col = this.gridSize.cols; col < newCols; col++) {
        this.grid[row].push({
          occupied: false,
          isActive: false,
        })
    ...
    this.gridSize.cols = newCols
    this.renderGrid()
    this.updateStatus(`Expanded to ${newCols} columns`)
    this.saveToStorage()
  }
  ...
  private saveToStorage() {
    try {
      // Save the raw grid state and grid size
      ...
      if (typeof PortIndependentStorage !== 'undefined') {
        PortIndependentStorage.save(storageData)
      } else {
        // Fallback to regular localStorage
        localStorage.setItem('musicComposer_data', JSON.stringify(storageData))
      }
    } catch (error) {
  ...
  private loadFromStorage() {
    try {
      ...
      // Try port-independent storage first
      if (typeof PortIndependentStorage !== 'undefined') {
        storageData = PortIndependentStorage.load()
      }

      // Fallback to regular localStorage
      if (!storageData) {
        const saved = localStorage.getItem('musicComposer_data')
        if (saved) {
          storageData = JSON.parse(saved)
        }
      }

      // Handle legacy format (just grid data)
      if (storageData && Array.isArray(storageData)) {
        this.loadGridFromData(storageData)
      } else if (storageData && storageData.grid) {
        // New format with grid size
        this.gridSize = storageData.gridSize || { rows: MusicComposer.NOTE_RANGE, cols: 64 }
        this.loadGridFromData(storageData.grid)
      } else {
